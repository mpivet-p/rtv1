...
/*   Updated: 2019/08/04 03:44:38 by mpivet-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_objects	*list_test(void)
{
	...
	while (i < 5)
	{
		...
		{
			ft_putstr_fd("Malloc error: test.c", 2);
			exit(1);
		}
		i++;
	}
	...
	ptr[0]->u_fig->sphere.color = 0xf54611;

	// Sphere 2
	ptr[1]->type = RT_SPHERE;
	ptr[1]->u_fig->sphere.pos = init_vector(-0.5, 1, 0);
	ptr[1]->u_fig->sphere.radius = 0.2;
	ptr[1]->u_fig->sphere.color = 0xf5a511;

	// Plane 1
	ptr[2]->type = RT_PLANE;
	ptr[2]->u_fig->plane.pos = init_vector(0, 0, 1);
	ptr[2]->u_fig->plane.dir = init_vector(0, 0, -0.5);
	ptr[2]->u_fig->plane.color = 0x0589f5;

	// Cone 1
	ptr[3]->type = RT_CONE;
	ptr[3]->u_fig->cone.pos = init_vector(0, 1, 0);
	ptr[3]->u_fig->cone.dir = init_vector(0, 0, 1);
	ptr[3]->u_fig->cone.height = 3;
	ptr[3]->u_fig->cone.radius = 1;
	ptr[3]->u_fig->cone.color = RT_GREEN;
